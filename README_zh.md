...
这是调查论文“Foundation Models in Robotics: Applications, Challenges, and the Future”的合作伙伴存储库。作者希望这个存储库可以作为方便机器人专家阅读相关论文并实施的快速参考。本自述文件的组织遵循论文中的图 1（如上所示），因此分为已应用于机器人技术的基础模型和与机器人技术有某种相关的基础模型。

<!-- We welcome contributions to this repository to add more resources. Please submit a pull request if you want to contribute! -->

我们欢迎对此存储库做出贡献以添加更多资源。如果您想做出贡献，请提交拉取请求！
...
在LLM+Robotics领域，常见的方法是先用language grounding将自然语言与现实世界中的实体和事件建立关联，计算 **Affordances** 以获得机器人在与特定环境交互时可以采取的具体能力或行为。例如，一个机器人需要检测到地面的摩擦力、地形的高度和形状等信息，才能决定如何行走或攀爬。**Affordance**源自心理学和认知科学的术语，最早由心理学家James J. Gibson提出，用于描述环境所允许个人能实现的功能，此术语常见于人机交互领域如界面设计等等。随着机器人智能的发展，**Affordance** 被迁移到机器人学科，强调的是智能体通过感知外界输入，以决定对物体有哪些可以被执行的操作。在机器人学中，**Affordance** 被推广至target, actions 和 effects三者之间的关系，即对target实行action以产生effect。
...
* Affordances in Robotic Tasks - A Survey [[Paper]](https://arxiv.org/pdf/2004.07400)
